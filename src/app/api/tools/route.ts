...
export function GET(request: NextRequest) {
  ...
  return ok({ items: listItems("tools", user.id, request.nextUrl.searchParams) });
}
...
export async function POST(request: NextRequest) {
  ...
  if (response) return response;
  try {
    return ok({ item: createItem("tools", user.id, toolSchema.parse(await request.json())) }, { status: 201 });
  } catch (error) {
