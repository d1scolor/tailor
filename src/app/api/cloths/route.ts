...
export function GET(request: NextRequest) {
  ...
  return ok({ items: listItems("cloths", user.id, request.nextUrl.searchParams) });
}
...
export async function POST(request: NextRequest) {
  ...
  if (response) return response;
  try {
    return ok({ item: createItem("cloths", user.id, clothSchema.parse(await request.json())) }, { status: 201 });
  } catch (error) {
