...
export function GET(request: NextRequest) {
  ...
  return ok({ items: listMeta("material_categories", user.id) });
}
...
export async function POST(request: NextRequest) {
  ...
  if (response) return response;
  try {
    return ok({ item: upsertMeta("material_categories", user.id, metaSchema.parse(await request.json())) }, { status: 201 });
  } catch (error) {
